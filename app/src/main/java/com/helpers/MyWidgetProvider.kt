...
import com.outstarttech.kabir.property.activities.StandardCal
...
class MyWidgetProvider : AppWidgetProvider(), Calculator {
    ...
    private fun performUpdate(context: Context) {
        ...
        appWidgetManager.getAppWidgetIds(getComponentName(context)).forEach {
            val views = RemoteViews(context.packageName, R.layout.activity_standard_cal)
    ...
    private fun setupAppOpenIntent(context: Context, views: RemoteViews, id: Int) {
        val intent = Intent(context, StandardCal::class.java)
    ...
    private fun myAction(action: String, context: Context) {
        if (calc == null) {
            calc = CalculatorImpl(StandardCal(), context)
        }
