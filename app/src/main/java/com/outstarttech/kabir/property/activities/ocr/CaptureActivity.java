package com.outstarttech.kabir.property.activities.ocr;
...
import com.outstarttech.kabir.property.activities.ocr.camera.CameraManager;
import com.outstarttech.kabir.property.activities.ocr.camera.ShutterButton;
import com.outstarttech.kabir.property.activities.ocr.language.LanguageCodeHelper;
...
public class CaptureActivity extends Activity implements SurfaceHolder.Callback,
    ...
    boolean handleOcrDecode(OcrResult ocrResult) {
        ...
        equation = ocrResult.getText();
        setClassVariable();

        if(isValidEquation()){
            Intent data = new Intent();
            data.putExtra(TextBlockObject, ocrResult.getText());
    ...
    */
    ...
    public boolean isValidEquation(){
        ...
        if(equation.length() == 0){
            ocrResultTextView.setText("You did not input anything!");
            return false;
        }
        if(variable.equals("None")){
            ocrResultTextView.setText("Invalid equation! There is no variable.");
            return false;
        }
        if (!equation.contains("=")){
            ocrResultTextView.setText("Invalid equation! There is no equality   sign in your equation");
            return false;
        }
        ...
        for(int j = 0; j < equation.length(); j++){
            if (equation.charAt(j) == '=') {
                ...
                if (numOfEqualitySign > 1) {
                    ocrResultTextView.setText("Invalid equation! You have more than one equality sign in your equation");
                    return false;
                }
                numOfEqualitySign +=1;
            }
            ...
            if(Character.isLetter(equation.charAt(j)) && !Character.toString(equation.charAt(j)).equals(variable)) {
                ocrResultTextView.setText("this is a linear equation solver you cant have more than one variable!");
                return false;
            }
        }

        if(equation.charAt(equation.length()-1) == '='){
            ocrResultTextView.setText("Invalid equation! You did not input anything after the equality sign(=)");
            return false;

        }

        else if(equation.charAt(0) == '='){
            ocrResultTextView.setText("Invalid equation! You did not input anything before the equality sign(=)");
            return false;
        }
        ...
        for(int k = 0; k < equation.length(); k++) {
            ...
            if (equation.charAt(k) == '-' || equation.charAt(k) == '+'){
                ...
                if(k == equation.length() -1){
                    ...
                    ocrResultTextView.setText("Invalid equation! Two or more signs(e.g +, -) can not be together ");
                    return true;
                }
            }
            if(Character.isLetter(equation.charAt(k))){
                if(k != equation.length() -1){
                    ...
                    if(Character.isLetter(equation.charAt(k+1))){
                        ocrResultTextView.setText("Invalid equation! Two or more  variables can not be together. ");
                        return false;
                    }
                }
            }
            if(k != equation.length() -1){
                ...
                {
                    ocrResultTextView.setText("Invalid equation! You are expected to input either \"+\" or \"-\"  sign after " +  equation.charAt(k) + " but you input " + equation.charAt(k+1) + " which is a number");
                    return false;
                }
            }
            if(equation.charAt(k) == '(' || equation.charAt(k) == ')'){
                ...
                if(k != equation.length() -1){
                    if(equation.charAt(k+1) == '(' || equation.charAt(k+1) == ')'){
                        ocrResultTextView.setText("Invalid equation! You cant have two or more " + sign + " together");
                        return false;
                    }
                }
            }
        ...
        for(int m = 0; m < equation.length(); m++) {
            ...
            if(ovar > 1){
                ocrResultTextView.setText("Invalid equation! You are expected to put a close bracket \")\" after " + equation.charAt(m-1) + " not an open bracket \"(\" ");
                return false;
            }
            else if (ovar < 0){
                ocrResultTextView.setText("Invalid equation! You are expected to put a open bracket \"(\" after " + equation.charAt(m-1) + " not a close bracket \") \" ");
                return false;
            }

        }
        if(ovar == 1){
            ocrResultTextView.setText("Invalid equation! You didnt close a bracket you opened.");
            return false;
        }
        String hold = "";
            ...
            if(equation.charAt(i) == '('){
                ...
                for(int q = 0; q  < num.length(); q++){
                    if(Character.isLetter(num.charAt(q))){
                        ocrResultTextView.setText("Invalid equation! You are not allowed to open a bracket with a variable");
                        return false;
                    }
                }
                for(int q = 0; q  < hold.length(); q++){
                    if(hold.charAt(q) == '='){
                        ocrResultTextView.setText("Invalid equation! It does not make sense to have an equality sign inside a bracket");
                        return false;
                    }
                    if(hold.length() == 1 ){
                        if(hold.charAt(q) == '+' || hold.charAt(q) == '-'){
                            // TextView textView = findViewById(R.id.);
                            ocrResultTextView.setText("Invalid equation! It is wrong to have only " + hold.charAt(q) + " in a bracket");
                            return false;
                        }
                    }
                }
    ...
    void displayProgressDialog() {
        ...
        String ocrEngineModeName = getOcrEngineModeName();
        if (ocrEngineModeName.equals("Both")) {
            indeterminateDialog.setMessage("Scanning Equation using S0PFiA Photomath...");
        } else {
            indeterminateDialog.setMessage("Scanning Equation using S0PFiA Photomath... ");
        }
        indeterminateDialog.setCancelable(false);
